...
export class RoleService {
    ...
    async create({ name, permissions }: createroleDto) {
        ...
        if (exists) {
            throw new NotFoundException(errorResponse(`Peran ${name} sudah ada`, 'ROLE_EXISTS', 400));
        }
        ...
        if (permissions.length > 0) {
            ...
            if (validPermissions.length !== permissions.length) {
                throw new NotFoundException(errorResponse('Satu atau lebih ID izin tidak valid', 'INVALID_PERMISSIONS', 400));
            }
        }
